...
## Script Name: export_account_address.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for account_address's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy merusepro.account_address to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo "account_address data export end time is : $taskEndTime "
exit 0
