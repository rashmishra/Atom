...
## Script Name: export_account_dwhid_mapping.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for account_dwhid_mapping's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy merusepro.account_dwhid_mapping to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo "account_dwhid_mapping data export end time is : $taskEndTime "
exit 0
