...
## Script Name: export_account_gst_details.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for account_gst_details's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy merusepro.account_gst_details to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo "account_gst_details data export end time is : $taskEndTime "
exit 0
