...
## Script Name: export_bpm_process.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for bpm_process's ETL run. Content will be appended in further tasks of T and L.
...
echo "bpm process data export start time is : $taskStartTime "
...
v_command="\copy act_hi_procinst to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for bpm_process data export is $v_extract_pid\n\n";
...
echo "bpm_process data export end time is : $taskEndTime "
exit 0