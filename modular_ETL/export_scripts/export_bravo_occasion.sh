...
## Script Name: export_bravo_occasion.sh
...
./mongoexport -u nbqa -p nearbuy --host "$v_datasource_ip" --db nb-marketing -c occasion --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "bravo_occasion mongo export end time is : $taskEndTime "
