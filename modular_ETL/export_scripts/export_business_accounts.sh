...
## Script Name: export_business_accounts.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for business_accounts's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy merusepro.business_accounts to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo "business_accounts data export end time is : $taskEndTime "
exit 0