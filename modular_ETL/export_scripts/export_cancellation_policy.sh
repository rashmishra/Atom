...
## Script Name: export_cancellation_policy.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for cancellation_policy's ETL run. Content will be appended in further tasks of T and L.
...
v_datasource_ip="nb-qa-db-nb-lookup.nbtools.com"
echo "${v_data_object}:datasource IP is $v_datasource_ip";

./mongoexport -u nbqa -p nearbuy --host "$v_datasource_ip" --db nb-lookup -c cancellationpolicy --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "cancellation_policy mongo export end time is : $taskEndTime "
