...
## Script Name: export_category_form.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for category_form's ETL run. Content will be appended in further tasks of T and L.
...
#v_log_obj_txt+=$(`./mongoexport --host 10.2.1.227:27017 --db cave_lookupplatform -q "$query" -c cave_lookup --out $v_data_dump_dir/$v_data_object.json` | tee /dev/tty )
#v_log_obj_txt=$(./mongoexport --host 10.2.1.227:27017 --db cave_lookupplatform -q "$query" -c cave_lookup --out $v_data_dump_dir/$v_data_object.json 2>&1)
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db cave_lookupplatform -q "$query" -c cave_lookup --out $v_data_dump_dir/$v_data_object.json 2>&1)`;
...
v_secondary_ip="nb-qa-db-nb-lookup.nbtools.com"
echo "${v_data_object}: Secondary IP is $v_secondary_ip";


./mongoexport --host "$v_secondary_ip" --db nb-lookup -c categoryForm --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
# Creating new file for cateogry_form's ETL run. Content will be appended in further tasks of T and L.
echo -e "$v_log_obj_txt" > $v_temp_dir/"$v_data_object"_log.log
chmod 0777 $v_temp_dir/"$v_data_object"_log.log
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
...
#echo "category_form mongo export end time is : $taskEndTime "
