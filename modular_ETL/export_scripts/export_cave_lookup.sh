...
## Script Name: export_cave_lookup.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for cave_lookup's ETL run. Content will be appended in further tasks of T and L.
...
./mongoexport --host "$v_secondary_ip" --db nb-lookup -c lookup --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "cave_lookup mongo export end time is : $taskEndTime "
