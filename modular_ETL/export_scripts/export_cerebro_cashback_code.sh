...
## Script Name: export_cerebro_cashback_code.sh
...
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db cerebro_cashback_codeplatform -q "$query" -c cerebro_cashback_code --out $v_data_dump_dir/$v_data_object.json 2>&1)`;
...
./mongoexport --host "$v_secondary_ip"  --db cerebro -c cashbackCode -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "cerebro_cashback_code mongo export end time is : $taskEndTime "
