...
## Script Name: export_cerebro_ucb.sh
...
query="{\$or:[{\"createdAt\":{\$gte:$v_incremental_epoch}},{\"updatedAt\":{\$gte:$v_incremental_epoch}}]}"

v_log_obj_txt+=`echo "\n$(date) Query is $query."`;


## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db cerebro_ucbplatform -q "$query" -c cerebro_ucb --out $v_data_dump_dir/$v_data_object.json 2>&1)`;
...
./mongoexport --host "$v_secondary_ip"  --db cerebro -c programSummary --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "cerebro_ucb mongo export end time is : $taskEndTime "
