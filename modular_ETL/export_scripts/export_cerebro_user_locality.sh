...
## Script Name: export_cerebro_user_locality.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for user_locality's ETL run. Content will be appended in further tasks of T and L.
...
query="{\$or:[{\"lastChecked\":{\$gte:$v_incremental_epoch}},{\"enteredAt\":{\$gte:$v_incremental_epoch}}]}"
...
./mongoexport --host "$v_secondary_ip" --db locationTracker -c userLocality -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "user_locality mongo export end time is : $taskEndTime "
