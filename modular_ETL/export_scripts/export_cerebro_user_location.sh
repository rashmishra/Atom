...
./mongoexport --host "$v_secondary_ip" --db locationTracker -c userLocation -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
