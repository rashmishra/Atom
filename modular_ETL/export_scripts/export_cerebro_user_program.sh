...
## Script Name: export_cerebro_user_program.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for cerebro_user_program's ETL run. Content will be appended in further tasks of T and L.
...
query="{\"time\":{\$gte:$v_incremental_epoch}}"

v_log_obj_txt+=`echo "\n$(date) Query is $query."`;


## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db cerebro_user_programplatform -q "$query" -c cerebro_user_program --out $v_data_dump_dir/$v_data_object.json 2>&1)`;

cd $v_mongo_dir


v_arbiter_ip="nb-prod-db-cerebro.nbtools.com"
...
#./mongoexport --host nb-prod-db-cerebro.nbtools.com:27017 
./mongoexport --host "$v_secondary_ip"  --db cerebro -c userProgram -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "cerebro_user_program mongo export end time is : $taskEndTime "
