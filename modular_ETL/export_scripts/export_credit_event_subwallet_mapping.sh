...
## Script Name: export_credit_event_subwallet_mapping.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for credit_event_subwallet_mapping's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy (SELECT * FROM nbcredit.crediteventsubwalletmapping WHERE createdat >= $v_incremental_epoch OR lastupdatedat >= $v_incremental_epoch ) to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo "Command to fetch credit_event_subwallet_mapping is: ${v_command}";

psql -d $DBNAME -h $DBHOST -p $DBPORT -U $DBUSER -A --field-separator=, -c "$v_command"  &
v_extract_pid=$!

echo -e "\n\nThe PID for credit_event_subwallet_mapping data export is $v_extract_pid\n\n";
...
echo "credit_event_subwallet_mapping data export end time is : $taskEndTime "
exit 0
