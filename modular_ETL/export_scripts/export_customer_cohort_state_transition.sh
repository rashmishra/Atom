...
## Script Name: export_customer_cohort_state_transition.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for customer_cohort_state_transition's ETL run. Content will be appended in further tasks of T and L.
...
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db categoryplatform -q "$query" -c category --out $v_data_dump_dir/$v_data_object.json 2>&1)`;

cd $v_mongo_dir


v_arbiter_ip="10.2.3.17"
...
./mongoexport --host "$v_secondary_ip"  --db cCohort  -c stateTransition --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "category mongo export end time is : $taskEndTime "
