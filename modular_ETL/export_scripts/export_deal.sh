...
v_datasource_ip="nb-qa-db-dealplatform.nbtools.com"
./mongoexport -u nbqa -p nearbuy --host "$v_datasource_ip" --db dealplatform -c deal -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
