...
## Script Name: export_expire_credits_status.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for expire_credits_status's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy nbcredit.expirecreditsstatus to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for expire_credits_status data export is $v_extract_pid\n\n";
...
echo "expire_credits_status data export end time is : $taskEndTime "
exit 0
