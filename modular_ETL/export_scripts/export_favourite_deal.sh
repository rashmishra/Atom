...
## Script Name: export_favourite_deal.sh
...
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db favourite_dealplatform -q "$query" -c favourite_deal --out $v_data_dump_dir/$v_data_object.json 2>&1)`;
...
./mongoexport --host "$v_secondary_ip" --db nearbuy_customer_deal_preferences  -c userPreferences --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "favourite_deal mongo export end time is : $taskEndTime "
