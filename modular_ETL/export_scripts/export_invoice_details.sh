...
## Script Name: export_invoice_details.sh
## Purpose: Modular ETL flow of Atom.

##### $1: Data Object. ####
##### $2: Data dump invoice_details. ####
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for invoice_details's ETL run. Content will be appended in further tasks of T and L.
...
primary=$(./mongo --host 10.2.1.235 --port 27017 --eval "printjson(rs.isMaster())" | grep "primary" | cut -d"\"" -f4) && echo $primary
v_prefix="ip-"
primary=${primary#$v_prefix}
v_primary_ip=${primary//-/.}
echo "Primary IP is $v_primary_ip"

./mongoexport --host "$v_primary_ip" --db aps-payable  -c invoice_details --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "invoice_details mongo export end time is : $taskEndTime "
