...
## Script Name: export_location.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for location's ETL run. Content will be appended in further tasks of T and L.
...
#v_log_obj_txt+=$(`./mongoexport --host 10.2.1.227:27017 --db locationplatform -q "$query" -c location --out $v_data_dump_dir/$v_data_object.json` | tee /dev/tty )
#v_log_obj_txt=$(./mongoexport --host 10.2.1.227:27017 --db locationplatform -q "$query" -c location --out $v_data_dump_dir/$v_data_object.json 2>&1)
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db locationplatform -q "$query" -c location --out $v_data_dump_dir/$v_data_object.json 2>&1)`;

cd $v_mongo_dir
./mongoexport --host 10.2.3.15:27017 --db location -c geoData  --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "location mongo export end time is : $taskEndTime "
