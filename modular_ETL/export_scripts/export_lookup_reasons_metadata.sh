...
## Script Name: export_lookup_reasons_metadata.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for ratingsAndReview's ETL run. Content will be appended in further tasks of T and L.
...
echo "Export command is ./mongoexport --host \"$v_secondary_ip\"  --db nearbuy_ratings_and_reviews -c lookUpReasonsMetaData --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/\"$v_data_object\"_extract_command_output.txt ";

./mongoexport --host "$v_secondary_ip"  --db nearbuy_ratings_and_reviews -c lookUpReasonsMetaData --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "ratingsAndReview mongo export end time is : $taskEndTime "
