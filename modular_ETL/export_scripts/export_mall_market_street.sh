...
## Script Name: export_mall_market_street.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for mall_market_street's ETL run. Content will be appended in further tasks of T and L.
...
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db mall_market_streetplatform -q "$query" -c mall_market_street --out $v_data_dump_dir/$v_data_object.json 2>&1)`;

cd $v_mongo_dir

./mongoexport --host 10.2.3.15:27017 --db location -c mall_market_street --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "mall_market_street mongo export end time is : $taskEndTime "
