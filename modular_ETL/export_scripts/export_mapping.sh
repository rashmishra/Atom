...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for mapping's ETL run. Content will be appended in further tasks of T and L.
...
#v_log_obj_txt+=$(`./mongoexport --host 10.2.1.227:27017 --db mappingplatform -q "$query" -c mapping --out $v_data_dump_dir/$v_data_object.json` | tee /dev/tty )
#v_log_obj_txt=$(./mongoexport --host 10.2.1.227:27017 --db mappingplatform -q "$query" -c mapping --out $v_data_dump_dir/$v_data_object.json 2>&1)
## Correct one
...
secondary=$(~/mongo_cp/bin/mongo --host 10.2.1.126 --port 27017 --eval "printjson(rs.status())" | tail -n+3 | grep -v "ISODate" | grep -v "Timestamp");
v_secondary_ip=$(echo "$secondary" | jq .members[1].name);
v_secondary_ip=${v_secondary_ip//\"/};
echo "Secondary IP is $v_secondary_ip"

./mongoexport --host "$v_secondary_ip" --db nb-mapping -c mapping  --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "mapping mongo export end time is : $taskEndTime "
