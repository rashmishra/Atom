...
3echo "${v_data_object}: Secondary IP is $v_secondary_ip";

v_datasource_ip="nb-qa-db-nb-mapping.nbtools.com"
./mongoexport --host "$v_datasource_ip" --db nb-mapping -c mapping  --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
