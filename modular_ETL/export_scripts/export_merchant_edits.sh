...
## Script Name: export_merchant_edits.sh
...
query="{\$or:[{\"raisedOn\":{\$gte:$v_incremental_epoch}},{\"resolvedOn\":{\$gte:$v_incremental_epoch} }]}";
v_log_obj_txt+=`echo "\n$(date) Query is $query."`;
...
./mongoexport --host "$v_secondary_ip" -d groupon_fox -c externalEditRequest -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "merchant_edits mongo export end time is : $taskEndTime "
