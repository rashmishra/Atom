...
## Script Name: export_merchant_process.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for merchant_process's ETL run. Content will be appended in further tasks of T and L.
...
echo "bpm merchant_process data export start time is : $taskStartTime "
...
v_command="\copy merchant_process to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for merchant_process data export is $v_extract_pid\n\n";
...
echo "merchant_process data export end time is : $taskEndTime "
exit 0