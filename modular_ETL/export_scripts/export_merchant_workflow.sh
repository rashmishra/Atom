...
## Script Name: export_merchant_workflow.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for merchant_workflow's ETL run. Content will be appended in further tasks of T and L.
...
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db groupon_fox -q "$query" -c merchant_workflow --out $v_data_dump_dir/$v_data_object.json 2>&1)`;

cd $v_mongo_dir

# v_arbiter_ip="10.20.1.190"
...
v_secondary_ip="nb-qa-db-groupon_fox.nbtools.com";

echo "${v_data_object}: Secondary IP is $v_secondary_ip";



./mongoexport --host "$v_secondary_ip" -u nbqa -p nearbuy --db groupon_fox  -c merchant_workflow --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "merchant_workflow mongo export end time is : $taskEndTime "
