...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for message's ETL run. Content will be appended in further tasks of T and L.
...
./mongoexport --host "$v_secondary_ip" --ssl --authenticationDatabase admin --username crmMongoAdmin --password crmM0ng0Admin --db nb-delivery-manager -c message -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "message mongo export end time is : $taskEndTime "
