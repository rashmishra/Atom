...
## Script Name: export_message_status_history.sh
...
v_data_object=$1;
v_data_dump_dir=$2
v_mongo_dir='/home/ubuntu/mongodb3-4-ssl/bin';
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for message_status_history's ETL run. Content will be appended in further tasks of T and L.
...
v_secondary_ip="mongodb://prod-crm-1-shard-00-00-adowb.mongodb.net:27017,prod-crm-1-shard-00-01-adowb.mongodb.net:27017,prod-crm-1-shard-00-02-adowb.mongodb.net:27017/test?replicaSet=prod-crm-1-shard-0";

echo "${v_data_object}: Secondary IP is $v_secondary_ip";

./mongoexport --host "$v_secondary_ip" --ssl --authenticationDatabase admin --username crmMongoAdmin --password crmM0ng0Admin --db nb-delivery-manager -c message_status_history -q "$query" --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "message_status_history mongo export end time is : $taskEndTime "
