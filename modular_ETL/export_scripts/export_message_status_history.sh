...
./mongoexport --host "$v_primary_ip" --db nb-delivery-manager -c message_status_history -q $query --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
