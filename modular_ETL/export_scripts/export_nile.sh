...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for nile's ETL run. Content will be appended in further tasks of T and L.
...
./mongoexport --host "$v_secondary_ip" --db nile -c transformeddeal --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "nile mongo export end time is : $taskEndTime "
