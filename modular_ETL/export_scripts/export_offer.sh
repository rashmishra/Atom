...
#v_log_obj_txt+=$(`./mongoexport --host 10.2.1.227:27017 --db offerplatform -q "$query" -c offer --out $v_data_dump_dir/$v_data_object.json` | tee /dev/tty )
#v_log_obj_txt=$(./mongoexport --host 10.2.1.227:27017 --db offerplatform -q "$query" -c offer --out $v_data_dump_dir/$v_data_object.json 2>&1)
## Correct one
...
./mongoexport --host "$v_secondary_ip" --db dealplatform -q "$query" -c deal --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "offer mongo export end time is : $taskEndTime "
