...
## Script Name: export_order_line_event.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for order_line_event's ETL run. Content will be appended in further tasks of T and L.
...
#echo "select * from oms_data.orderlineevent where createdat>$v_incremental_epoch or updatedat>$v_incremental_epoch" > ./query_$v_data_object.txt
...
v_command="\copy (select * from oms_data.orderlineevent where createdat>$v_incremental_epoch or updatedat>$v_incremental_epoch or createdat IS NULL)  to $v_extract_filename with DELIMITER ',' CSV HEADER"
