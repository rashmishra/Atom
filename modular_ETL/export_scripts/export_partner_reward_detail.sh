...
## Script Name: export_reward_detail.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for reward_detail's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy (select * from rs.partnerrewarddetail where createdat>$v_incremental_epoch or updatedat>$v_incremental_epoch)  to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for Partner Reward Details data export is $v_extract_pid\n\n";
