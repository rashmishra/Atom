...
## Script Name: export_payment_terms.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for payment_terms's ETL run. Content will be appended in further tasks of T and L.
...
#v_log_obj_txt+=$(`./mongoexport --host 10.2.1.227:27017 --db payment_termsplatform -q "$query" -c payment_terms --out $v_data_dump_dir/$v_data_object.json` | tee /dev/tty )
#v_log_obj_txt=$(./mongoexport --host 10.2.1.227:27017 --db payment_termsplatform -q "$query" -c payment_terms --out $v_data_dump_dir/$v_data_object.json 2>&1)
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db payment_termsplatform -q "$query" -c payment_terms --out $v_data_dump_dir/$v_data_object.json 2>&1)`;
...
./mongoexport --host "$v_secondary_ip" -u nbqa -p nearbuy --db nb-lookup -c paymentTerms --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "payment_terms mongo export end time is : $taskEndTime "
