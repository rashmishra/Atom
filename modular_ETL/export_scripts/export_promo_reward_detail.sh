...
v_command="\copy (select * from rs.promorewarddetails where createdat>$v_incremental_epoch or updatedat>$v_incremental_epoch)  to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for Promo Reward Details data export is $v_extract_pid\n\n";
