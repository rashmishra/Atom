...
## Script Name: export_transaction.sh
...
v_command="\copy (select * from oms_data.transaction where createdat>$v_incremental_epoch or updatedat>$v_incremental_epoch)  to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo "Transaction data export end time is : $taskEndTime "
exit 0