...
## Script Name: export_user_credit_summary.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for user_credit_summary's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy nbcredit.usercreditsummary to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for user_credit_summary data export is $v_extract_pid\n\n";
...
echo "user_credit_summary data export end time is : $taskEndTime "
exit 0
