...
## Script Name: export_user_device_token_status.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for user_device_token_status's ETL run. Content will be appended in further tasks of T and L.
...
## Correct one
#v_export_result=`echo $(./mongoexport --host 10.2.1.227:27017 --db user_device_token_statusplatform -q "$query" -c user_device_token_status --out $v_data_dump_dir/$v_data_object.json 2>&1)`;
...
./mongoexport --host "$v_secondary_ip" --ssl --authenticationDatabase admin --username crmMongoAdmin --password crmM0ng0Admin --db nb-delivery-manager -c user_device_token_status --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
#echo "user_device_token_status mongo export end time is : $taskEndTime "
