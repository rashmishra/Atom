...
## Script Name: export_user_mapped_locations.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for user_mapped_locations's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy merusepro.user_mapped_locations to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for user_mapped_locations data export is $v_extract_pid\n\n";
...
echo "user_mapped_locations data export end time is : $taskEndTime "
exit 0