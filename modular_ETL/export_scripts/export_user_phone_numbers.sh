...
## Script Name: export_user_phone_numbers.sh
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for user_phone_numbers's ETL run. Content will be appended in further tasks of T and L.
...
v_command="\copy merusepro.user_phone_numbers to $v_extract_filename with DELIMITER ',' CSV HEADER"
...
echo -e "\n\nThe PID for user_phone_numbers data export is $v_extract_pid\n\n";
...
echo "user_phone_numbers data export end time is : $taskEndTime "
exit 0