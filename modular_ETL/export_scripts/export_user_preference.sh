...
secondary=$(~/mongo_cp/bin/mongo --host 10.2.3.113 --port 27017 --eval "printjson(rs.status())" | tail -n+3 | grep -v "ISODate" | grep -v "Timestamp");
v_secondary_ip=$(echo "$secondary" | jq .members[1].name);
v_secondary_ip=${v_secondary_ip//\"/};

echo "Secondary IP is $v_secondary_ip"

./mongoexport --host "$v_secondary_ip" --db notification_preference  -c user_preference --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
