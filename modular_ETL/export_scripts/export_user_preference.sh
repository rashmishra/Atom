...
primary=$(~/mongo_cp/bin/mongo --host 10.2.3.113 --port 27017 --eval "printjson(rs.status())" | tail -n+3 | grep -v "ISODate" | grep -v "Timestamp");
v_primary_ip=$(echo "$primary" | jq .members[1].name);
v_primary_ip=${v_primary_ip//\"/};

echo "Primary IP is $v_primary_ip"

./mongoexport --host "$v_primary_ip" --db notification_preference  -c user_preference --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
