...
## Script Name: export_user_requests_offer.sh
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for user_requests_offer's ETL run. Content will be appended in further tasks of T and L.
...
./mongoexport --host "$v_secondary_ip" --db nearbuy_customer_deal_preferences -c userRequestsOffer --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
