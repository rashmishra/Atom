...
## Script Name: export_promocode.sh
## Purpose: Modular ETL flow of Atom.

##### $1: Data Object. ####
##### $2: Data dump promocode. ####
...
p_exit_upon_error(){
    ...
    then
        ...
        # Creating new file for promocode's ETL run. Content will be appended in further tasks of T and L.
...
v_arbiter_ip="nb-prod-fooddeliverydb-rs1.nbtools.com"
...
./mongoexport --host $v_secondary_ip -vvvv -d food-delivery -c ZFDOrder --out $v_data_dump_dir/$v_data_object.json 2> $v_temp_dir/"$v_data_object"_extract_command_output.txt &
v_extract_pid=$!
...
# Creating new file for zomato food delivery's ETL run. Content will be appended in further tasks of T and L.
echo -e "$v_log_obj_txt" > $v_temp_dir/"$v_data_object"_log.log
chmod 0777 $v_temp_dir/"$v_data_object"_log.log
#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#-#
...
#echo "zomato food delivery mongo export end time is : $taskEndTime "
