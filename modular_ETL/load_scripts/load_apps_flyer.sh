...
## Script Name: load_apps_flyer.sh
...
schemaFileName=schema_apps_flyer.json
maxBadRecords=0
...
echo "In apps_flyer Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for apps_flyer's ETL run. Content will be appended in further tasks of T and L.
...
echo "Etl Home is $6."
echo "Schema File path for Apps Flyer is: $v_schema_filepath"
...
if [[ "`bq ls $v_dataset_name | awk '{print $1}' | grep \"\b$tableName\b\"`" == "$tableName" ]] ;
    then 
        ...
        v_query="SELECT * FROM $v_dataset_name.$tableName WHERE event_time_epoch < (SELECT SELECT MIN(event_time_epoch) as oldest_event_epoch FROM $v_metadataset_name.incremental_$tableName)";
...
echo "Apps Flyer Load status is : $v_task_status"
