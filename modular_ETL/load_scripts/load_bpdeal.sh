...
schemaFileName=schema_bpdeal.json
maxBadRecords=0
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for bpdeal's ETL run. Content will be appended in further tasks of T and L.
...
bq load --quiet --source_format=NEWLINE_DELIMITED_JSON --replace --ignore_unknown_values=1 --max_bad_records=$maxBadRecords $v_dataset_name.$tableName $v_cloud_storage_path/$v_fileName $v_schema_filepath/$schemaFileName &
