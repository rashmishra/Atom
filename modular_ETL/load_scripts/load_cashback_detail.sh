...
## Script Name: load_cashback_detail.sh
...
echo "In cashback_detail Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for cashback_detail's ETL run. Content will be appended in further tasks of T and L.
...
schemaFileName=schema_cashback_detail.json
maxBadRecords=0
...
if [[ "`bq ls $v_dataset_name | awk '{print $1}' | grep \"\b$tableName\b\"`" == "$tableName" ]] ;
    then 
            ...
            bq query  --maximum_billing_tier 10 --allow_large_results=1 --quiet --replace -n 1 --destination_table=$v_destination_tbl "$v_query" &
            v_pid=$!
