...
schemaFileName=schema_category.json
maxBadRecords=0
...
echo "In category Loading script";
...
if [[ "`bq ls $v_dataset_name | awk '{print $1}' | grep \"\b$tableName\b\"`" == "$tableName" ]] ;
	...
	then bq rm $v_destination_tbl;
        ...
        bq query --quiet   --maximum_billing_tier 10 --allow_large_results=1 --flatten_results=0 --replace --destination_table=$v_destination_tbl "$v_query" 2> "$v_data_object"_prior_table_result.txt &
        v_pid=$!
...
#bq query --append=1 --flatten_results=0 --allow_large_results=1 --destination_table=$METADATA_DATASET_NAME.final_category 'select * from '"$METADATA_DATASET_NAME"'.category_incremental' > /dev/null
