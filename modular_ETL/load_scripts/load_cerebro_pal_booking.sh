...
## Script Name: load_cerebro_pal_booking.sh
...
schemaFileName=schema_cerebro_pal_booking.json
maxBadRecords=0

v_data_object=$1;
tableName=pal_booking;
...
echo "In cerebro_pal_booking Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for cerebro_pal_booking's ETL run. Content will be appended in further tasks of T and L.
...
cd $v_load_dir;
echo "In Load directory, from cerebro_pal_booking script $v_load_dir";
...
###################################################################################
echo "cerebro_pal_booking's Load status is: $v_task_status" 
echo $v_task_status > $v_temp_dir/${v_data_object}_load_status.txt
