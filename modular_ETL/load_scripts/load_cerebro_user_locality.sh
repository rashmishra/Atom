...
## Script Name: load_cerebro_user_locality.sh
...
schemaFileName=schema_cerebro_user_locality.json
maxBadRecords=0

v_data_object=$1;
# Configuration object name is different from the tablename in BQ
tableName=user_locality;
...
echo "In cerebro_user_locality Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for cerebro_user_locality's ETL run. Content will be appended in further tasks of T and L.
...
###################################################################################
echo "cerebro_user_locality's Load status is: $v_task_status" 
echo $v_task_status > $v_temp_dir/${v_data_object}_load_status.txt
