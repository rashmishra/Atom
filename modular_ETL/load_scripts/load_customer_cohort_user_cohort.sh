...
## Script Name: load_customer_cohort_user_cohort.sh
...
schemaFileName=schema_customer_cohort_user_cohort.json
maxBadRecords=100
...
echo "In customer_cohort_user_cohort Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for customer_cohort_user_cohort's ETL run. Content will be appended in further tasks of T and L.
...
bq load --replace --quiet --field_delimiter=',' --source_format=CSV --skip_leading_rows=1 --max_bad_records=$maxBadRecords --allow_jagged_rows=1 --allow_quoted_newlines=1 --ignore_unknown_values=1 $v_destination_tbl $v_cloud_storage_path/$v_fileName $v_schema_filepath/$schemaFileName 2> "$v_data_object"_inc_table_result.txt
