...
## Script Name: load_item_key_store.sh
...
schemaFileName=schema_item_key_store.json
maxBadRecords=0
...
echo "In item_key_store Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for item_key_store's ETL run. Content will be appended in further tasks of T and L.
...
bq load --quiet --replace  --field_delimiter=',' --source_format=CSV --skip_leading_rows=1 --max_bad_records=0  --allow_jagged_rows=1 --allow_quoted_newlines=1 --ignore_unknown_values=1 $v_dataset_name.$tableName $v_cloud_storage_path/$v_fileName $v_schema_filepath/$schemaFileName &
