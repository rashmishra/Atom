...
## Script Name: load_lookup_reasons_metadata.sh
...
schemaFileName=schema_lookup_reasons_metadata.json
maxBadRecords=0
...
echo "In lookup_reasons_metadata Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for lookup_reasons_metadata's ETL run. Content will be appended in further tasks of T and L.
...
echo "Load Statement for lookup_reasons_metadata: bq load --quiet --source_format=NEWLINE_DELIMITED_JSON --replace --ignore_unknown_values=1 --max_bad_records=$maxBadRecords $v_destination_tbl $v_cloud_storage_path/$v_fileName $v_schema_filepath/$schemaFileName"
