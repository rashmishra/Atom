...
## Script Name: load_merchant_edits.sh
...
schemaFileName=schema_merchant_edits.json
maxBadRecords=0
...
echo "In merchant_edits Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for merchant_edits's ETL run. Content will be appended in further tasks of T and L.
...
if [[ "`bq ls --max_results=10000 $v_dataset_name | awk '{print $1}' | grep \"\b$tableName\b\"`" == "$tableName" ]] ;
    then 
        ...
        v_query="SELECT merchantId, raisedBy, raisedOn, raisedFrom, changeRequest.attribute
                        , changeRequest.resolution, changeRequest.resolvedBy
                        , changeRequest.resolvedOn, isRequestResolved, resolvedOn
                 FROM (SELECT *, CONCAT(STRING( merchantId), STRING(raisedOn))  as dkey FROM $v_dataset_name.$tableName) A
                 WHERE dkey NOT IN 
                                (SELECT CONCAT(STRING( merchantId), STRING(raisedOn)) as key
                                 FROM $v_metadataset_name.incremental_$tableName 
                                 GROUP BY key)";
