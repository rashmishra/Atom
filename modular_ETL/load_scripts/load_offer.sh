...
schemaFileName=schema_offer.json
maxBadRecords=0
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for offer's ETL run. Content will be appended in further tasks of T and L.
...
cd $v_load_dir;
echo "In Load directory, from Offer script $v_load_dir";
...
if [[ "`bq ls --max_results=10000 $v_dataset_name | awk '{print $1}' | grep \"\b$tableName\b\"`" == "$tableName" ]] ;
    then 
        ...
        v_query="SELECT * FROM $v_dataset_name.$tableName WHERE _id NOT IN (SELECT _id FROM $v_metadataset_name.incremental_$tableName)";
        v_destination_tbl="$v_metadataset_name.prior_$tableName";
        echo "Destination table is $v_destination_tbl and Query is $v_query"
        bq query  --maximum_billing_tier 10 --allow_large_results=1 -n 0 --quiet --flatten_results=0 --replace --destination_table=$v_destination_tbl "$v_query" 2> "$v_data_object"_prior_table_result.txt &
        v_pid=$!
...
###################################################################################
echo "Offer's Load status is: $v_task_status" 
echo $v_task_status > $v_temp_dir/${v_data_object}_load_status.txt
