...
## Script Name: load_order_header_status_history.sh
...
echo "In order_header_status_history Loading script";
...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        # Creating new file for Order Header status history's ETL run. Content will be appended in further tasks of T and L.
...
schemaFileName=schema_order_header_status_history.json
maxBadRecords=0
...
if [[ "`bq ls --max_results=10000 $v_dataset_name | awk '{print $1}' | grep \"\b$tableName\b\"`" == "$tableName" ]] ;
    then 
            ...
            v_query="SELECT * FROM $v_dataset_name.$tableName WHERE ohshid NOT IN (SELECT ohshid FROM $v_metadataset_name.incremental_$tableName)";
...
# Creating new file for order_header_status_history's ETL run. Content will be appended in further tasks of T and L.
echo -e "$v_log_obj_txt" >> $v_temp_dir/"$v_data_object"_log.log
