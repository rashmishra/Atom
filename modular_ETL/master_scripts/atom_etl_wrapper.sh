...
p_exit_upon_error(){
    ...
    if [ $v_task_status == "failed" ] ; then
        ...
        mail -s "PROD Machine | Task ($v_subtask) failed in Atom wrapper script"    snehesh.mitra@nearbuy.com    rashmi.mishra@nearbuy.com abhishek.manocha@nearbuy.com harsh.choudhary@nearbuy.com  < /dev/null ;
...
mail -s "PROD Machine | Atom ETL master script invoked."   snehesh.mitra@nearbuy.com   rashmi.mishra@nearbuy.com abhishek.manocha@nearbuy.com harsh.choudhary@nearbuy.com  < /dev/null
...
mail -s "PROD Machine | Atom ETL master script completed. Invoking Transformation of BOM to old OL"   snehesh.mitra@nearbuy.com   rashmi.mishra@nearbuy.com abhishek.manocha@nearbuy.com harsh.choudhary@nearbuy.com  < /dev/null
...
if [ "$v_status" == "success" ]; 
  then 
    echo `date` "Task: $v_subtask ended with status $v_task_status." | mail -s "PROD Machine | Wrapper script calling ETL and Transform OL completed: $v_task_status"   snehesh.mitra@nearbuy.com abhishek.manocha@nearbuy.com harsh.choudhary@nearbuy.com   rashmi.mishra@nearbuy.com ;
  else 
    echo `date` "Task: $v_subtask ended with status $v_task_status." | mail -s "PROD Machine | Wrapper script calling ETL and Transform OL completed: $v_task_status"   snehesh.mitra@nearbuy.com abhishek.manocha@nearbuy.com harsh.choudhary@nearbuy.com    rashmi.mishra@nearbuy.com ;
    exit 1;
fi
